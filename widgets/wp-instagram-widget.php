<?php
function wptravel_instagram_load_widget() {
	register_widget( 'wptravel_instagram_widget' );
}
add_action( 'widgets_init', 'wptravel_instagram_load_widget' );

class wptravel_instagram_widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'wptravel-instagram-feed',
			__( 'NBT Instagram', 'wp-instagram-widget' ),
			array(
				'classname' => 'wptravel-instagram-feed',
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) ) {
		    echo '<div class="instagram-title container txt-center">';
		...
		do_action( 'wptravel_before_widget', $instance );
		...
		if ( $username != '' ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				if ( $images_only = apply_filters( 'wptravel_images_only', FALSE ) ) {
				...
				$ulclass = apply_filters( 'wptravel_list_class', 'instagram-pics instagram-size-' . $size );
				$liclass = apply_filters( 'wptravel_item_class', '' );
				$aclass = apply_filters( 'wptravel_a_class', '' );
				$imgclass = apply_filters( 'wptravel_img_class', '' );
				$template_part = apply_filters( 'wptravel_template_part', 'parts/wp-instagram-widget.php' );
				...
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( $template_part ) != '' ) {
						...
						echo '<div class="'. esc_attr( $liclass ) .' wptravel-instagram col-md-2 pd0 col-sp-6 col-xs-4"><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"  class="'. esc_attr( $aclass ) .'"><img src="'. esc_url( $item[$size] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. esc_attr( $imgclass ) .'"/></a></div>';
					}
				}
				?></div><?php
			}
		...
		do_action( 'wptravel_after_widget', $instance );
